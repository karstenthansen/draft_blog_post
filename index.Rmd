...
library(knitr)
library(dplyr)
chunk_dir <- 'C:\\research\\variety\\blog\\chunks\\'
...
```{r plot.hypothetical.choices, fig.width=10, fig.height=5, echo=FALSE}
```
...
```{r plot.average.cr, fig.width=8, fig.height=5}
```
...
```{r plot.distribution.cr1, fig.width=8, fig.height=5}
```
